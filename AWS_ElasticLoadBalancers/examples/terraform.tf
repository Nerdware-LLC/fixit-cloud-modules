...
module "AWS_ELB" {
  source = "git@github.com:Nerdware-LLC/fixit-cloud-modules.git//AWS_ElasticLoadBalancers"
    ...
    Simple_ALB = {
      ...
      subnets = {
        FooPrivateSubnet_in_useast2a = {
          subnet_id = module.Foo_VPC.Subnets.FooPrivateSubnet_A.id
        }
        FooPrivateSubnet_in_useast2b = {
          subnet_id = module.Foo_VPC.Subnets.FooPrivateSubnet_B.id
        }
      }

      alb_security_group_ids = [
        module.Foo_VPC.SecurityGroups["ALB_SecGrp"].id
      ]
    }
  }
