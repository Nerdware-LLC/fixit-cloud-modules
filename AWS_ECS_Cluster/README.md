...
| <a name="input_appmesh_routes"></a> [appmesh\_routes](#input\_appmesh\_routes) | Map of AppMesh Route names to route config objects. "spec" must have a<br>"priority" between 0-1000 and a "type", which can be "http\_route", "http2\_route",<br>"tcp\_route", or "grpc\_route". | <pre>map(<br>    # map keys: AppMesh route names e.g. "serviceB-route"<br>    object({<br>      virtual_router_name = string<br>      type                = string # "http_route", "http2_route", "tcp_route", or "grpc_route"<br>      spec = object({<br>        action = string<br>        match = optional(object({<br>          method = string<br>          prefix = string<br>        }))<br>        retry_policy = string<br>        timeout      = number<br>      })<br>      tags = optional(map(string))<br>    })<br>  )</pre> | `null` | no |
