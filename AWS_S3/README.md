...
| <a name="input_bucket_policy"></a> [bucket\_policy](#input\_bucket\_policy) | The IAM bucket policy of the S3 bucket. If not provided, a bucket policy<br>will be created which enforces the use of SSE-related headers for all<br>PutObject operations. If provided, the value must be a JSON-encoded string<br>which can be properly converted into a valid bucket policy upon being passed<br>into the "jsondecode" Terraform function. The aforementioned SSE-related<br>headers will be merged into the resulting policy object. | `string` | `null` | no |
