...
module "CloudWatch_Alarm_SNS_Topic" {
  /* module inputs */
}
...
module "AWS_CloudWatch" {
  ...
  cloudwatch_logs_log_groups = {
    "Org_CloudTrail/Security/us-west-1" = {
      ...
      kms_key_arn       = module.CloudTrail_KMS_Key.KMS_Key.arn
    }
  }
  ...
  cloudwatch_metric_alarms = {
    ...
    UnauthorizedAPICalls = {
      ...
      alarm_action_arns   = [module.CloudWatch_Alarm_SNS_Topic.Topic.arn]
    }
      ...
      widgets = [
        {
          ...
          properties = {
            metrics = [
              ["AWS/EC2", "CPUUtilization", "InstanceId", module.EC2_Instance.EC2.id]
            ]
            period = 300
            stat   = "Average"
            region = "us-west-1"
