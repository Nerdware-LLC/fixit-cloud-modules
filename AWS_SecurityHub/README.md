...
<!-- TODO add module description to README here -->
...
| [aws_organizations_delegated_administrators.list](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/organizations_delegated_administrators) | data source |
...
|------|-------------|------|---------|:--------:|
| <a name="input_securityhub_member_accounts"></a> [securityhub\_member\_accounts](#input\_securityhub\_member\_accounts) | List of account param objects for all accounts within the<br>Organization. The Delegated Admin account needs this list to add<br>each account to the Org's SecurityHub service. Each account-param<br>object must include the account's ID and email. | <pre>map(object({<br>    id    = string<br>    email = string<br>  }))</pre> | `null` | no |
